...
from octree import ConstructTree
...
def RefineMin(q, Vmin, D, Dmin, shrink):
	...
	while D > Dmin:
		...
		filt = (arr>=hist[1][pid])*(arr<=(hist[1][pid+1]))
		if any(filt):
			Vmin = np.median(arr[filt])
		else:
			return Vmin
...
def morph(gal, j_circ_from_r=False, LogInterp=False, BoundOnly=False, Ecut=None, jThinMin=0.7, mode='tree', theta=0.7, quadrupole=False, dimcell='1 kpc', DumpProb=False):
	...
	theta -- opening angle of the tree to tune force computation accuracy when mode is 'tree'. Default is 0.7
...
#-------
	...
	# Calcolo il raggio come “2*(M_r - m_r)” e lo trasformo in stringa con unità kpc:
	radius = 2.0 * (M_r - m_r)           # valore in kpc, perché gal['r'] è in kpc
	d = gal[filt.Disc(f"{radius} kpc", gal['eps'].min())]
...
#-------
	...
	if mode == 'tree':
		tree_obj = ConstructTree(np.float64(gal['pos']), np.float64(gal['mass']), np.float64(gal['eps']), quadrupole=quadrupole)
	elif mode == 'pm':
...
#--------------------------------------------------------------------------------------------------------
	...
	if BoundOnly:
		...
		if len(g_star)<CutPart:
			print('WARNING: very few bound particles (%d)\n' % (len(g_star)))
...
#-------
		...
		if len(Ecut)==0:
			...
			if len(Ecut[rel_filt])==0:
				...
				#Exclude anyway those Ecut which leave too few particles in the low-energy component
				if (np.sum(mass[te<Ecut])/np.sum(mass)<Mmin):
					Ecut = 0	
			else:
